...
class MessagesController extends Controller
{
    ...
    public function index($id = null)
    {
        ...
        return view('Chatify::pages.app', [
            'id' => $id ?? 0,
            'messengerColor' => $messenger_color ?: Chatify::getFallbackColor(),
            'dark_mode' => $user && $user->dark_mode < 1 ? 'light' : 'dark',
        ]);
    }
    ...
    {
        $query = Chatify::fetchMessagesQuery($request['id'], Auth::guard('admin')->user()->id)->where('sent_by','shipper_admin')->latest();
    ...
    public function getContacts(Request $request)
    {
        ...
        $users = Message::join('customers',  function ($join) {
            ...
            ->where(function ($q) {
                $q->where('ch_messages.from_id', Auth::guard('admin')->user()->id)
                    ->orWhere('ch_messages.to_id', Auth::guard('admin')->user()->id);
            })
    ...
    {
        $favoritesList = null;
        $favorites = Favorite::where('user_id', Auth::guard('admin')->user()->id);
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($request['dark_mode']) {
            $request['dark_mode'] == "dark"
                ? Admin::where('id', Auth::guard('admin')->user()->id)->update(['dark_mode' => 1])  // Make Dark
                : Admin::where('id', Auth::guard('admin')->user()->id)->update(['dark_mode' => 0]); // Make Light
        }
        ...
        if ($request->hasFile('avatar')) {
            ...
            if ($file->getSize() < Chatify::getMaxUploadSize()) {
                if (in_array(strtolower($file->extension()), $allowed_images)) {
                    ...
                    if (Auth::guard('admin')->user()->avatar != config('chatify.user_avatar.default')) {
                    ...
                    $update = Admin::where('id', Auth::guard('admin')->user()->id)->update(['avatar' => $avatar]);
    ...
    public function setActiveStatus(Request $request)
    {
        ...
        $status = User::where('id', Auth::guard('admin')->user()->id)->update(['active_status' => $activeStatus]);
        return Response::json([
            'status' => $status,
        ], 200);
    }
